...
public class PLAYLIST_update_playlist extends TestBase{
	...
	requestParams.put("description", "updated description");
	requestParams.put("availability", "private");
...
//	httpRequest.header("Authorization-Token", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..MFCY0XAogSXQOO_Bid6Rd-RSC9nXGgkLfAwPgYffTd4");
	...
	response = httpRequest.request(Method.POST,"/v1/update_playlist");
...
//		Assert.assertEquals(responseBody.contains("kashi"), true);
//		Assert.assertEquals(responseBody.contains("music"), true);
		Assert.assertEquals(responseBody.contains(Integer.toString(ids)), true);
		Assert.assertEquals(responseBody.contains("playlist has been updated."), true);
...
void tearDown()
{
	logger.info("*********  Finished update_playlist **********");
}
